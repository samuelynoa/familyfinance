...
export default function GastosLista() {
  ...
  async function load() {
    ...
    try {
      ...
      // - Admin ve todo
      // - Miembro ve: sus gastos personales + todos los gastos familiares
      const visibles = isAdmin
        ? g
        : g.filter(gasto => {
            const esFamiliar  = gasto.personal_familiar === 'familiar' || !gasto.personal_familiar
            const esMio       = gasto.usuario_id === perfil.id
            return esFamiliar || esMio
          })
          ...
          {filtrados.map((g, i) => {
            ...
            return (
              <div key={g.id || i} style={{
                ...
                <div style={{ width:42, height:42, borderRadius:11, background:'var(--color-card-hover,#F3F4F6)',
