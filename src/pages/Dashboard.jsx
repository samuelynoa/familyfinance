...
export default function Dashboard() {
  ...
  // Balance total de cuentas operativas (no solo consulta)
      ...
      {ahorro !== 0 && (
        <div style={{
          ...
          <span style={{ fontWeight: 700 }}>
            {ahorro >= 0 ? '💚 Ahorro del mes:' : '⚠️ Déficit del mes:'}
          </span>
  ...
  return (
    <div className="card" style={{ padding: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem', marginBottom: '.5rem' }}>
        <div style={{ background: bg, color, borderRadius: 8, padding: '.3rem', display: 'flex' }}>
          {icon}
        </div>
...
function CuentaChip({ cuenta }) {
  return (
    <div style={{
      background: cuenta.color || '#2E6DA4',
      color: '#fff',
      borderRadius: 12,
      padding: '.7rem 1rem',
      minWidth: 140,
      ...
      <p style={{ fontWeight: 800, fontSize: '1rem' }}>
        {cuenta.moneda === 'USD' ? '$' : 'RD$'} {Number(cuenta.balance || 0).toLocaleString('es-DO', { minimumFractionDigits: 2 })}
      </p>
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'right' }}>
        <p style={{ fontWeight: 700, color: '#DC2626', fontSize: '.9rem' }}>
          -{gasto.monto_rdp ? `RD$${Number(gasto.monto_rdp).toLocaleString('es-DO')}` : `$${gasto.monto_usd}`}
        </p>
...
const styles = {
  balanceCard: {
    ...
    boxShadow: '0 8px 24px rgba(30,58,95,.35)',
  },
