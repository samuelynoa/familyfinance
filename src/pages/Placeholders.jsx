// Placeholder pages — will be fully built in Phase 2 & 5
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem', color: '#2E6DA4', fontSize: '.875rem' }}>
        <Construction size={16} /> Fase 2 — próximamente
      </div>
    </div>
  )
}
...
export function Config() {
  return (
    <div>
      ...
      <div className="card">
        <Link to="/config/usuarios" style={{ display: 'flex', alignItems: 'center', gap: '1rem', textDecoration: 'none', color: '#1F2937', padding: '.5rem 0' }}>
          <Settings size={20} color="#2E6DA4" />
          <span style={{ fontWeight: 600 }}>Gestión de usuarios</span>
        </Link>
      </div>
      <Placeholder icon={Settings} title="Más opciones" description="Presupuestos, cuentas, tarjetas y permisos — disponibles en Fase 2." color="#9CA3AF" />
    </div>
  )
}
