...
export async function getRecurrentes({ usuarioId = '', isAdmin = false } = {}) {
...
export async function addRecurrente(r) {
  ...
  await appendRow('gastos_recurrentes', { id, ...r })
  return id
}

export async function updateRecurrente(id, cambios) {
  const data = await getSheet('gastos_recurrentes')
  const idx  = (data.rows || []).findIndex(r => r.id === id)
  if (idx === -1) throw new Error('Recurrente no encontrado')
  await updateRow('gastos_recurrentes', idx + 2, { ...data.rows[idx], ...cambios })
}
