...
function extraerMonto(texto) {
  ...
  for (const patron of patrones) {
    const matches = [...texto.matchAll(new RegExp(patron.source, patron.flags))]
    for (const match of matches) {
      const valor = parseFloat(match[1].replace(/,/g, ''))
      if (!isNaN(valor) && valor > 0) {
        // Para TOTAL/MONTO/IMPORTE/A PAGAR tomamos el primero que encontremos
        if (patron.source.startsWith('total|monto|importe|a\\s\\*pagar')) {
          return { monto: valor, fuente: 'etiqueta' }
        }
        // Para el resto tomamos el valor más alto (suele ser el total)
        if (valor > mayorValor) {
          mayorValor = valor
          mejorMonto = valor
        }
