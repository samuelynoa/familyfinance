// All Sheets calls go through /api/sheets (Vercel serverless proxy)
// so the service account credentials never reach the browser.
...
export async function getGastos(filters = {}) {
  ...
  if (filters.mes) {
    rows = rows.filter(r => r.fecha && r.fecha.startsWith(filters.mes))
  }
...
export async function upsertComercio(nombre, categoria) {
  ...
  if (idx === -1) {
    ...
    await updateRow('comercios_aprendidos', idx + 2, updated)
  }
}
