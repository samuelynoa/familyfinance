    ...
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100dvh', background:'#F9FAFB' }}>
      ...
      <header style={{ position:'sticky', top:0, zIndex:90, background:'#fff', borderBottom:'1px solid #E5E7EB', padding:'.75rem 1rem', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'.5rem' }}>
          {/* Hide/show balances — persiste en localStorage */}
          <button
            onClick={toggleHideBalances}
            title={hideBalances ? 'Mostrar balances' : 'Ocultar balances'}
            style={{ background: hideBalances ? '#EEF5FC' : '#F3F4F6', border:'none', cursor:'pointer', padding:'.4rem .5rem', borderRadius:8, display:'flex', alignItems:'center', gap:'.3rem', color: hideBalances ? '#2E6DA4' : '#6B7280' }}>
            {hideBalances ? <EyeOff size={18}/> : <Eye size={18}/>}
            <span style={{ fontSize:'.72rem', fontWeight:600 }}>{hideBalances ? 'Oculto' : 'Visible'}</span>
          </button>

          {/* Avatar + menú */}
          <div style={{ position:'relative' }} ref={menuRef}>
            <button onClick={() => setMenuOpen(v => !v)} style={{ width:36, height:36, borderRadius:'50%', cursor:'pointer', background: perfil?.avatar_color||'#2E6DA4', border: menuOpen?'2px solid #2E6DA4':'2px solid transparent', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:'.8rem', outline:'none' }}>
              {initials}
            </button>

            {menuOpen && (
              <div style={{ position:'absolute', top:'calc(100% + .5rem)', right:0, background:'#fff', borderRadius:14, minWidth:200, boxShadow:'0 8px 32px rgba(0,0,0,.15)', border:'1px solid #E5E7EB', overflow:'hidden', zIndex:200 }}>
                <div style={{ padding:'.9rem 1rem', display:'flex', alignItems:'center', gap:'.75rem' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <p style={{ fontWeight:700, fontSize:'.9rem', color:'#1F2937', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                ...
                <button onClick={handleLogout} style={{ width:'100%', display:'flex', alignItems:'center', gap:'.75rem', padding:'.85rem 1rem', background:'none', border:'none', cursor:'pointer', fontSize:'.9rem', color:'#DC2626' }}>
